...
class BusinessCardDataTab extends StatefulWidget {
  const BusinessCardDataTab({super.key, required this.card, this.onDelete});
...
class BusinessCardDataTabState extends State<BusinessCardDataTab> {
  ...
  final List<
    ({
      TextEditingController formatted,
      AddressLabel label,
      String? customLabel,
      FocusNode focus,
    })
  ...
  Future<String?> _showCustomLabelDialog(
    String title,
    String? initialValue,
  ) async {
  ...
  bool _dataFieldsChanged(BusinessCard a, BusinessCard b) {
    ...
    if (a.contact.socialMedias.length != b.contact.socialMedias.length)
      return true;
    ...
    for (var i = 0; i < a.contact.emails.length; i++) {
      if (a.contact.emails[i].address != b.contact.emails[i].address)
        return true;
    }
    ...
    for (var i = 0; i < a.contact.socialMedias.length; i++) {
      if (a.contact.socialMedias[i].username !=
          b.contact.socialMedias[i].username)
        return true;
    }
    for (var i = 0; i < a.contact.addresses.length; i++) {
      if (a.contact.addresses[i].formatted != b.contact.addresses[i].formatted)
        return true;
    }
    final aBday = a.contact.events
        .where((e) => e.label.label == EventLabel.birthday)
        .firstOrNull;
    final bBday = b.contact.events
        .where((e) => e.label.label == EventLabel.birthday)
        .firstOrNull;
  ...
  void _syncControllersFromCard(BusinessCard card, [AppLocalizations? loc]) {
    ...
    final org = card.contact.organizations.isNotEmpty
        ? card.contact.organizations.first
        : null;
    _departmentController.text = org?.departmentName ?? card.displaySubtitle;
    _notesController.text = card.contact.notes.isNotEmpty
        ? card.contact.notes.first.note
        : '';

    _addedNameFields.clear();
    if ((card.contact.name?.prefix ?? '').isNotEmpty)
      _addedNameFields.add('prefix');
    if ((card.contact.name?.middle ?? '').isNotEmpty)
      _addedNameFields.add('middle');
    if ((card.contact.name?.suffix ?? '').isNotEmpty)
      _addedNameFields.add('suffix');
    if ((card.contact.name?.nickname ?? '').isNotEmpty)
      _addedNameFields.add('nickname');

    _hasDepartment = (org?.departmentName ?? card.displaySubtitle)
        .trim()
        .isNotEmpty;
    _hasJobTitle = (org?.jobTitle ?? card.displayTitle).trim().isNotEmpty;

    final bday = card.contact.events
        .where((e) => e.label.label == EventLabel.birthday)
        .firstOrNull;
    ...
    for (final p in card.contact.phones) {
      ...
      final (label, customLabel) = VcardLabelOptions.normalizePhoneLabel(
        p.label.label,
        p.label.customLabel,
        loc,
      );
    ...
    for (final e in card.contact.emails) {
      final fn = FocusNode();
      fn.addListener(() => setState(() {}));
      final (label, customLabel) = VcardLabelOptions.normalizeEmailLabel(
        e.label.label,
        e.label.customLabel,
        loc,
      );
    ...
    for (final w in card.contact.websites) {
      final fn = FocusNode();
      fn.addListener(() => setState(() {}));
      _websiteEntries.add((url: TextEditingController(text: w.url), focus: fn));
    }
    ...
    for (final a in card.contact.addresses) {
      final fn = FocusNode();
      fn.addListener(() => setState(() {}));
      final (label, customLabel) = VcardLabelOptions.normalizeAddressLabel(
        a.label.label,
        a.label.customLabel,
        loc,
      );
  ...
  BusinessCard getDraftCard() {
    ...
    for (final e in _socialMediaEntries) {
      ...
      if (s.isNotEmpty) {
        final label = SocialPlatformResolver.resolveForWrite(
          e.platform,
          e.customLabel,
        );
    ...
    final hasOrgData =
        orgName.isNotEmpty || jobTitle.isNotEmpty || department.isNotEmpty;
    final organizations = hasOrgData
        ? <Organization>[
            Organization(
              name: orgName.isNotEmpty ? orgName : null,
    ...
    final notes = notesText.isNotEmpty
        ? <Note>[Note(note: notesText)]
        : <Note>[];
    ...
    final primaryPhone = phones.isNotEmpty
        ? phones.first.number
        : (_phoneEntries.isNotEmpty
              ? _phoneEntries.first.number.text.trim()
              : '');
    final primaryEmail = emails.isNotEmpty
        ? emails.first.address
        : (_emailEntries.isNotEmpty
              ? _emailEntries.first.address.text.trim()
              : '');
  ...
  Widget _buildAddButtonsRow(
    List<({String label, VoidCallback onAdd})> buttons,
  ) {
  ...
  void _addWebsite() {
    ...
    fn.addListener(() => setState(() {}));
    setState(
      () => _websiteEntries.add((url: TextEditingController(), focus: fn)),
    );
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  static const _denseFieldPadding = EdgeInsets.symmetric(
    horizontal: 12,
    vertical: 14,
  );
          ...
          children: [
            ...
            _buildAddButtonsRow([
              if (!_addedNameFields.contains('prefix'))
                (
                  label: loc.honorificPrefix,
                  onAdd: () => _addNameField('prefix', _prefixFocus),
                ),
              if (!_addedNameFields.contains('middle'))
                (
                  label: loc.additionalNames,
                  onAdd: () => _addNameField('middle', _middleFocus),
                ),
              if (!_addedNameFields.contains('suffix'))
                (
                  label: loc.honorificSuffix,
                  onAdd: () => _addNameField('suffix', _suffixFocus),
                ),
              if (!_addedNameFields.contains('nickname'))
                (
                  label: loc.nickname,
                  onAdd: () => _addNameField('nickname', _nicknameFocus),
                ),
              if (!_hasBirthday) (label: loc.birthday, onAdd: _addBirthday),
            ]),
            ...
            TextField(
              ...
              decoration: InputDecoration(labelText: loc.organization),
            ),
            if (_hasJobTitle) ...[
              const SizedBox(height: _fieldSpacing),
              TextField(
            ...
            _buildAddButtonsRow([
              if (!_hasJobTitle) (label: loc.addJobTitle, onAdd: _addJobTitle),
              if (!_hasDepartment)
                        ...
                        items: VcardLabelOptions.phoneLabels
                            .map(
                              (l) => DropdownMenuItem(
                                value: l,
                                child: Text(
                                  VcardLabelOptions.phoneDisplayName(
                                    l,
                                    loc,
                                    e.label == l ? e.customLabel : null,
                                  ),
                                ),
                              ),
                            )
                        ...
                        onChanged: (l) async {
                          if (l != null) {
                            if (l == PhoneLabel.custom) {
                              final custom = await _showCustomLabelDialog(
                                loc.phoneLabel,
                                e.customLabel,
                              );
                              if (!mounted) return;
                    ...
                    Expanded(
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          labelText: loc.phone,
                          errorText: hasError
                              ? loc.pleaseSpecifyLabelForCustom
                              : null,
                        ),
            ...
            const SizedBox(height: _fieldSpacing),
            _buildSectionAddButton(label: loc.addPhone, onPressed: _addPhone),
            const SizedBox(height: _addButtonToSection),
                        ...
                        items: VcardLabelOptions.emailLabels
                            .map(
                              (l) => DropdownMenuItem(
                                value: l,
                                child: Text(
                                  VcardLabelOptions.emailDisplayName(
                                    l,
                                    loc,
                                    e.label == l ? e.customLabel : null,
                                  ),
            ...
            const SizedBox(height: _fieldSpacing),
            _buildSectionAddButton(label: loc.addEmail, onPressed: _addEmail),
            const SizedBox(height: _addButtonToSection),
            ...
            const SizedBox(height: _fieldSpacing),
            _buildSectionAddButton(label: loc.addUrl, onPressed: _addWebsite),
            const SizedBox(height: _addButtonToSection),
                  ...
                  children: [
                    SizedBox(
                      ...
                      child: DropdownButtonFormField<AppSocialPlatform?>(
                        key: ValueKey(
                          'social-$i-${e.platform?.id}-${e.customLabel ?? ''}',
                        ),
                        ...
                        items: [
                          ...SocialPlatformResolver.appPlatforms.map(
                            (p) => DropdownMenuItem<AppSocialPlatform?>(
                              value: p,
                              child: Text(
                                SocialPlatformResolver.displayName(p),
                              ),
                            ),
                          ),
                            ...
                            final custom = await showDialog<String>(
                              ...
                              builder: (ctx) {
                                var controller = TextEditingController(
                                  text: _socialMediaEntries[i].customLabel,
                                );
                                return AlertDialog(
                  ...
                  children: [
                    ...
                    SizedBox(
                      ...
                      child: DropdownButtonFormField<AddressLabel>(
                        key: ValueKey(
                          'address-$i-${e.label}-${e.customLabel ?? ''}',
                        ),
                        initialValue: e.label,
                        isExpanded: true,
                        style: TextStyle(
                        ...
                        items: VcardLabelOptions.addressLabels
                            .map(
                              (l) => DropdownMenuItem(
                                value: l,
                                child: Text(
                                  VcardLabelOptions.addressDisplayName(
                                    l,
                                    loc,
                                    e.label == l ? e.customLabel : null,
                                  ),
